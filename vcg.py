...
class VCG :
	...
	def call(self, t, v) :
		...
		dx_dt = -np.sum((self.w * self.alpha_x / (self.b_x**2)) * dtheta_x * np.exp(-dtheta_x**2 / (2 * (self.b_x**2))))
		dy_dt = -np.sum((self.w * self.alpha_y / (self.b_y**2)) * dtheta_y * np.exp(-dtheta_y**2 / (2 * (self.b_y**2))))
		dz_dt = -np.sum((self.w * self.alpha_z / (self.b_z**2)) * dtheta_z * np.exp(-dtheta_z**2 / (2 * (self.b_z**2))))
